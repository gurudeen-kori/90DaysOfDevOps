...
- [ ] Three separate VPCs, each with a distinct CIDR range
- [ ] Three EC2 instances, each with the correct instance type for its environment
- [ ] `Name` tags read `terraweek-dev-server`, `terraweek-staging-server`, `terraweek-prod-server`
